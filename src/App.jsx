...
function normalisePosition(line) {
  const c = cleanLine(line).toUpperCase();
  return isLikelyPosition(c) ? c : "";
}
...
function parseOCRText(text) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (isLikelyName(line) && isLikelyPosition(next)) {
      players.push({ id: makeId(), no: String(players.length+1), name: line, pos: normalisePosition(next) }); i++; continue;
    }
    if (isLikelyName(line) && /^(CONFIRMED|SELECTED)$/i.test(next) && isLikelyPosition(next2)) {
      players.push({ id: makeId(), no: String(players.length+1), name: line, pos: normalisePosition(next2) }); i+=2; continue;
    }
...
function getPositionSlot(players) {
  const grouped = players.reduce((acc, p) => {
    const k = (p.pos||"CM").toUpperCase().trim(); (acc[k]=acc[k]||[]).push(p); return acc;
  }, {});
  const placed = [];
...
function Button({ children, variant="primary", ...props }) {
  ...
  return <button {...props} style={{...s[variant],borderRadius:14,padding:"10px 14px",fontWeight:600,cursor:props.disabled?"not-allowed":"pointer",opacity:props.disabled?0.55:1}}>{children}</button>;
}
...
function MobileTabBar({ activeTab, setActiveTab }) {
  return (
    <nav style={{
      ...
      zIndex:1000, boxShadow:"0 -4px 20px rgba(0,0,0,0.08)",
      ...
      {MOBILE_TABS.map(({ key, label, Icon }) => {
        ...
        return (
          <button key={key} onClick={() => setActiveTab(key)} style={{
            ...
            cursor:"pointer", color: active ? "#0f172a" : "#94a3b8",
            position:"relative", minHeight:60, paddingBottom:4,
          }}>
    ...
    <div style={{display:"flex",gap:8,marginBottom:4}}>
      {tabs.map(([k,l]) => (
        <button key={k} onClick={()=>setActive(k)} style={{
          ...
          background: active===k ? "#0f172a" : "#e2e8f0",
          color: active===k ? "#fff" : "#64748b",
      ...
      {(!isMobile || sub==="photo") && (
        <Card>
          ...
          <CardContent>
            ...
            {ocrError && (
              <div style={{border:"1px solid #fde68a",background:"#fefce8",color:"#854d0e",borderRadius:16,padding:14,marginBottom:16}}>
                <div style={{display:"flex",alignItems:"flex-start",gap:8}}><AlertTriangle size={16} style={{marginTop:2}}/><span>{ocrError}</span></div>
              </div>
            )}
            <div style={{border:"1px solid #e2e8f0",background:"#f8fafc",borderRadius:16,padding:12,minHeight:260}}>
              {imageUrl
                ...
                : <div style={{minHeight:220,display:"flex",alignItems:"center",justifyContent:"center",border:"1px dashed #cbd5e1",borderRadius:12,color:"#64748b"}}>Team sheet preview</div>
              }
            </div>
          </CardContent>
        </Card>
            ...
            <div style={{display:"grid",gap:16}}>
              {(!isMobile || sub==="ocr") && (
                <div>
                  ...
                  <Textarea value={ocrText} onChange={(e)=>setOcrText(e.target.value)} placeholder="Paste OCR output from Google Lens…" rows={6}/>
                </div>
              )}
              {(!isMobile || sub==="fix") && (
                <>
                  <div>
                    <div style={{marginBottom:8,fontSize:14,fontWeight:600}}>Manual Clean-up</div>
                    <Textarea value={manualText} onChange={(e)=>setManualText(e.target.value)} placeholder="Player name on one line, position on next." rows={10}/>
                  </div>
                  ...
                  <div style={{border:"1px solid #bbf7d0",background:"#f0fdf4",color:"#166534",borderRadius:16,padding:14,fontSize:13}}>
                    💡 Best mobile flow: photo → Google Lens → copy → paste here → parse → fix names.
                  </div>
                </>
              )}
            </div>
...
function PitchTab({ isMobile, placedPlayers, selectedPlayerId, setSelectedPlayerId, events, players, leaderboard, actionButtons, logAction, undoLastEvent, resetMatch }) {
    ...
    <div style={{display:"grid",gap:16,gridTemplateColumns:isMobile?"1fr":"minmax(0,1.15fr) minmax(320px,0.85fr)"}}>
      <Card>
        <CardHeader>
          ...
          {/* Mobile sub-tabs inside card header */}
        ...
        <CardContent>
          ...
          {(!isMobile || sub==="pitch") && (
            <>
              <div style={{
                position:"relative",margin:"0 auto",width:"100%",maxWidth:isMobile?340:420,
                aspectRatio:"0.72/1",overflow:"hidden",borderRadius:28,border:"4px solid #fff",
                ...
                {placedPlayers.map((p) => {
                  ...
                  return (
                    <motion.button key={p.id} whileTap={{scale:0.92}} onClick={()=>setSelectedPlayerId(p.id)} style={{
                      position:"absolute",left:`${p.x}%`,top:`${p.y}%`,transform:"translate(-50%,-50%)",
                      width:isMobile?48:56,height:isMobile?48:56,borderRadius:"50%",
                      border:sel?"3px solid #0f172a":"2px solid #fff",
                      background:sel?"#fcd34d":"#fff",color:"#0f172a",
                      display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",
                      boxShadow:sel?"0 0 0 3px rgba(252,211,77,0.4),0 8px 18px rgba(0,0,0,0.2)":"0 4px 12px rgba(0,0,0,0.18)",
                      cursor:"pointer",
                    }}>
              ...
              {/* Action buttons — 4 col grid fits 7 buttons neatly (4+3) */}
              <div style={{marginTop:14,display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:8}}>
                {actionButtons.map(({key,icon:Icon,label})=>(
                  <Button key={key} disabled={!selectedPlayerId} variant="outline" onClick={()=>logAction(key)}>
                    <div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:3}}><Icon size={16}/><span style={{fontSize:11}}>{label}</span></div>
                  </Button>
                ))}
              </div>
            </>
...
export default function App() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      setEvents(s.events||[]); setDb(s.db||[]); setMatchName(s.matchName||"U13 Match");
  ...
  useEffect(() => {
    localStorage.setItem(STORAGE_KEY, JSON.stringify({players,events,db,matchName,opponent,ocrText,manualText,ocrError}));
  }, [players,events,db,matchName,opponent,ocrText,manualText,ocrError]);

  useEffect(() => {
    if (!timerOn) return; const id=setInterval(()=>setClock((c)=>c+1),1000); return ()=>clearInterval(id);
  }, [timerOn]);
  ...
  // ── Players ──────────────────────────────────────────────────────────────
  const addBlankPlayer=()=>setPlayers(prev=>sortPlayersByNumber([...prev,{id:makeId(),no:String(prev.length+1),name:"New Player",pos:"CM"}]));
  ...
  // ── Events ───────────────────────────────────────────────────────────────
  const logAction    =(key)=>{if(!selectedPlayerId)return;setEvents(prev=>[{id:makeId(),t:clock,playerId:selectedPlayerId,action:key,ts:new Date().toISOString()},...prev]);};
  const undoLastEvent=()=>setEvents(prev=>prev.slice(1));
  const resetMatch   =()=>{setEvents([]);setClock(0);setTimerOn(false);setSelectedPlayerId(null);};
  const finishMatch  =()=>{
    const summary=leaderboard.map(({id:playerId,no,name,pos,score,counts})=>({playerId,no,name,pos,score,counts}));
    setDb(prev=>[{id:makeId(),matchName,opponent,playedAt:new Date().toISOString(),durationSeconds:clock,summary,events},...prev]);
  };

  // ── Exports ──────────────────────────────────────────────────────────────
  function exportJSON(){const blob=new Blob([JSON.stringify({players,events,db,matchName,opponent},null,2)],{type:"application/json"});const a=Object.assign(document.createElement("a"),{href:URL.createObjectURL(blob),download:`scout-${matchName.replace(/\s+/g,"-").toLowerCase()}.json`});a.click();URL.revokeObjectURL(a.href);}
  function exportCSV(){const rows=[["Time","No","Player","Pos","Action"],...events.map(e=>{const p=players.find(x=>x.id===e.playerId),a=ACTIONS.find(x=>x.key===e.action);return[formatClock(e.t),p?.no||"",p?.name||"",p?.pos||"",a?.label||e.action];})];const csv=rows.map(r=>r.map(v=>`"${String(v).replace(/"/g,'""')}"`).join(",")).join("\n");const blob=new Blob([csv],{type:"text/csv;charset=utf-8;"});const a=Object.assign(document.createElement("a"),{href:URL.createObjectURL(blob),download:`events-${matchName.replace(/\s+/g,"-").toLowerCase()}.csv`});a.click();URL.revokeObjectURL(a.href);}
  ...
  return (
    <div style={{
      ...
      paddingBottom: isMobile ? 80 : 16,   // room for bottom tab bar
      color:"#0f172a",
      ...
      <div style={{maxWidth:1320,margin:"0 auto",display:"grid",gap:16}}>
          ...
          <Card>
            ...
            <CardContent>
              <div style={{display:"grid",gap:12,gridTemplateColumns:isMobile?"1fr 1fr":"repeat(auto-fit,minmax(200px,1fr))"}}>
                ...
                <div style={{gridColumn:isMobile?"1/-1":undefined,display:"flex",justifyContent:"space-between",alignItems:"center",gap:12,padding:14,border:"1px solid #e2e8f0",borderRadius:16,background:"#f8fafc"}}>
                  <div><div style={{fontSize:12,color:"#64748b"}}>Match Clock</div><div style={{fontSize:28,fontWeight:800}}>{formatClock(clock)}</div></div>
                  <div style={{display:"flex",gap:8}}>
                    <Button variant="outline" onClick={()=>setTimerOn(s=>!s)}>{timerOn?"Pause":"Start"}</Button>
                ...
                {selectedPlayer ? (
                  <div style={{display:"grid",gap:12}}>
                    ...
                    <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:8}}>
                      {ACTIONS.map((a)=>(
                        <div key={a.key} style={{border:"1px solid #e2e8f0",borderRadius:14,background:"#f8fafc",padding:10,textAlign:"center"}}>
                          <div style={{fontSize:18}}>{a.icon}</div><div style={{fontSize:12,color:"#64748b"}}>{a.label}</div>
                          <div style={{fontWeight:700}}>{events.filter(e=>e.playerId===selectedPlayer.id&&e.action===a.key).length}</div>
                        </div>
                      ))}
                    </div>
                  </div>
        ...
        {activeTab==="pitch"    && <PitchTab    isMobile={isMobile} placedPlayers={placedPlayers} selectedPlayerId={selectedPlayerId} setSelectedPlayerId={setSelectedPlayerId} events={events} players={players} leaderboard={leaderboard} actionButtons={actionButtons} logAction={logAction} undoLastEvent={undoLastEvent} resetMatch={resetMatch}/>}
